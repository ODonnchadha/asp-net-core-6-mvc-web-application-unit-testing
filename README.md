...
- IMTEGRATING UNIT TESTS IN DEVELOPMENT & RELEASE FLOWS:
    - Alternative ways of running tests.
        ...
        - Test runners versus test frameworks:
            - e.g.: xUnit is a test runner.
            - Test runner: Responsible for looking for one or more assemblies with tests in then and activating the test frameworks that it finds in those assemblies.
              - e.g.: xunit.runner.visualstudio. Remove this and the project will still build.
            - Test framework: The code that has detailed knowledge of how to discover & run unit tests. e.g.: xunit.
    - Integration:
        - Parallel testing: Allows a set of tests to finish faster, locally or on the build server.
            - A test runner can support running different test assemblies in parallel.
                - xunit, by default, runs different test classes as an unique test collection. 
                    - Class tests will not run in parallel with each other. Tests within different test classes will run in parallel.
                    - Tests within the same collection do not run in parallel, even if they are within different collections.
            - A test framework can support running tests within a single assembly in parallel.
            ```csharp
                [Fact()]
                public void Slow()
                {
            ...
            ```
        - Parallelism within a test runner: All assemblies in parallel or none at all. (Current issue in VS 2019.)
        - Testing against multiple target frameworks.
            - xUnit supports running in multiple target frameworks. e.g.: older class library.
                - e.g.: netstandard 2.1 target framework.
                ```csharp
                    <TargetFramework>net6.0</TargetFramework>
                    <ImplicitUsings>true</ImplicitUsings>
                ```
                ```csharp
                    <TargetFrameworkss>net6.0;net5.0</TargetFrameworkss>
                ```
        - Integrating testing in CI/CD pipeline:
            1. Run tests locally. Commit code.
            2. Compile on build server. Report results.
            3. Apply quality control. Unit. Function. Automated. Code coverage. Performance.
            4. Deploy the artifact.
            - Azure Pipeline. Edit pipeline. 
                - Add a task that executes the unit tests. Note: Visual Studio Tests. (Catch all.)
                - With xUnit: Select dotnet build. After build. Before publish.
                    - Path to project: 
                    ```csharp
                        **/EmployeeManagement.Test/*.csproj
                    ```
    - SUMMARY:
        - Test runner: Program responsible for looking for 1:M assemblies with tests in them & activaying the test frameworks that is finds in those assemblies.
        - Test framework: Code that has detailed knowledge of how to discover and run unit tests.
        - Run tests with the CLI via dotnet test.